...
<%
	ComVO comup = (ComVO) request.getAttribute("cv");
...
<body>
	...
	<!-- 확인 -->
		<form action="./update.do" method="post" enctype="multipart/form-data">
			<input type="hidden" name="com_num" value="<%=comup.getComNum() %>" >
			<div class="city">
				<input type="text" id="city" class="title-input" name="city_name" placeholder="도시를 입력해 주세요" value="<%=comup.getCityName() %>" />
			</div>

			<div class="title">
				<input type="text" id="title" class="title-input" name=com_title placeholder="제목을 입력해 주세요" value="<%=comup.getComTitle()%>"/>
			</div>

			<div class="content">
				<textarea name="contents" id="summernote"><%=comup.getComCont()%></textarea>
				
<!-- 				<div> -->
<!-- 				<label for="content">내용</label> -->
				
<%-- 				<textarea id="content" name="com_cn" rows="10" placeholder="내용를 입력하세요." ><%=comup.getComCont()%></textarea> --%>
				
<!-- 				<button onclick="insertImage()" class="img_put">이미지 삽입</button> -->
<!-- 				</div> -->
				<!-- <a class="btn-link" href="./list.do" onclick="handleLinkClick()">저장</a> -->
				<button id="writeBtn" name="button" value="issue" type="submit">수정</button>
			</div>
<!-- 			<input class="btn-link" type="submit" onclick="handleLinkClick()" value="저장"> -->
		</form>
	</div>
	...
	$(document).ready(function() {
		...
		function insertImage() {
			...
			if (imageUrl) {
				var content = document.getElementById('content');
				var imageTag = '<img src="' + imageUrl + '" alt="이미지">';
