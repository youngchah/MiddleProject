...
<%
	...
	List<ComVO> myCommuList = (List<ComVO>)request.getAttribute("myCommuList");
...
<body>
    ...
    <div class="infoAndDelete">
        <button id="memberInfoBtn">나의 커뮤니티</button>
...
<%
if (myCommuList != null && !myCommuList.isEmpty()) { 
	
   	for (ComVO cv : myCommuList) {
%>
<div class="log-container" id="log-container-<%=cv.getComNum()%>">
        
        <!-- 로그 리스트 아이템 -->
        <div class="log-list">
    <a class="log-link" rel="noopener noreferrer" href="<%=request.getContextPath()%>/community/detail.do?com_num=<%=cv.getComNum()%>" style="text-decoration: none;">
        <div class="log-container-inner" style="display: flex;">
            ...
            <div class="log-content-container" style="flex: 1;">
                ...
                <div class="log-category-container">
                    <div class="log-category-date"><%=cv.getComRegi()%></div>
                </div>

                <!-- 로그 제목 -->
                <div style="font-size: 20px; font-weight: bold;" class="log-title"><%=cv.getComTitle() %></div>

                <!-- 작성자 -->
                <div class="log-author"><%=cv.getMemNick() %></div>
            </div>
...
<%-- 		<a href="<%=request.getContextPath() %>/community/detail.do?com_num=<%=cv.getComNum() %> "style="font-size: 30px;"> <%=cv.getComTitle()%> </a> --%>
...
//      var logContDoc = parser.parseFromString(logContHtml, 'text/html');
     ...
     var logContainerId = 'log-container-' + '<%=cv.getComNum()%>';
