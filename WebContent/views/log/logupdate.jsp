...
<%
	...
	LogVO logDetail = logDetailList.get(0);
%>
...
<body>
	...
	<form method="post" action="<%=request.getContextPath() %>/log/logupdate.do" enctype="multipart/form-data">
		<div class="logContainer">

			<div class="header">

				<input type="text" class="title-input" name="cityName" placeholder="도시를 입력해 주세요" 
				value="<%=logDetail.getCityName()%>"/>
				<input type="text" id="datepicker" placeholder="날짜 선택" 
				value="<%=logDetail.getLogStart()%> to <%=logDetail.getLogEnd()%>"/> 
				<input type="hidden" name="startDate" id="hiddenStartDate" value="<%=logDetail.getLogStart()%>">
				<input type="hidden" name="endDate" id="hiddenEndDate" value="<%=logDetail.getLogEnd()%>">
				<input type="hidden" name="logNum" value="<%=logDetail.getLogNum() %>" >
				<input type="text" class="title-input" name="logTitle" placeholder="제목을 입력해 주세요"
                    value="<%= logDetail.getLogTitle() %>">
			...
			<div class="align_center">
				...
				<textarea name="contents" id="summernote" ><%= logDetail.getLogCont() %></textarea>
